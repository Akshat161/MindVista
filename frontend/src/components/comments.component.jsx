...
export const fetchComments =async({skip=0,blod_id,setParentCommentCountFun,comment_array = null})=>{
...
let{blog:{_id,title,comments:{results:commentsArr},activity:{total_parent_comments}} ,commentsWrapper,setCommentsWrapper,totalParentCommentsLoaded,setTotalParentCommentsLoaded }=useContext(BlogContext)

const loadMoreComments=async()=>{
    let newCommentsArr = await fetchComments({
        skip:totalParentCommentsLoaded,blog_id:_id,setParentCommentCountFun:setTotalParentCommentsLoaded,comment_array: commentsArr
    })
    ...
    return(
        <div className={"max-sm:w-full fixed"+(commentsWrapper?"top-0 sm:right-0" : "top-[100%] sm:right-[-100%]")+" duration-700 max-sm:right-0 sm:top-0 w-[30%] min-w-[350px] h-full z-50 bg-white shadow-2xl p-8 px-16 overflow-y-auto overflow-x-hidden"}>
            ...
            <div className="relative">
                ...
                <p className="text-lg mt-2 w-[70%] text-dark-grey line-clamp-1">{title}
                </p>
        ...
        }
        {
            ...
            className="text-dark-grey p-2 px-3 hover:bg-grey/30rounded ">
                Load More
            </button>
            :""
        }
